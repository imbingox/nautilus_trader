...
EXTRA_REEXPORTS: dict[str, tuple[str, ...]] = {
    "nautilus_trader/analysis/__init__.pyi": (
        ...
        (
            "from nautilus_trader.analysis.config import TearsheetBarsWithFillsChart as "
        ...
        (
            "from nautilus_trader.analysis.config import TearsheetDistributionChart as "
            "TearsheetDistributionChart"
        ),
        (
            "from nautilus_trader.analysis.config import TearsheetDrawdownChart as "
            "TearsheetDrawdownChart"
        ),
        "from nautilus_trader.analysis.config import TearsheetEquityChart as TearsheetEquityChart",
        (
            "from nautilus_trader.analysis.config import TearsheetMonthlyReturnsChart as "
            "TearsheetMonthlyReturnsChart"
        ),
        (
            "from nautilus_trader.analysis.config import TearsheetRollingSharpeChart as "
            "TearsheetRollingSharpeChart"
        ),
        (
            "from nautilus_trader.analysis.config import TearsheetRunInfoChart as "
            "TearsheetRunInfoChart"
        ),
        (
            "from nautilus_trader.analysis.config import TearsheetStatsTableChart as "
            "TearsheetStatsTableChart"
        ),
        (
            "from nautilus_trader.analysis.config import TearsheetYearlyReturnsChart as "
        ...
        (
            "from nautilus_trader.analysis.tearsheet import create_drawdown_chart as "
            "create_drawdown_chart"
        ),
        "from nautilus_trader.analysis.tearsheet import create_equity_curve as create_equity_curve",
        (
            "from nautilus_trader.analysis.tearsheet import create_monthly_returns_heatmap as "
            "create_monthly_returns_heatmap"
        ),
        (
            "from nautilus_trader.analysis.tearsheet import create_returns_distribution as "
            "create_returns_distribution"
        ),
        (
            "from nautilus_trader.analysis.tearsheet import create_rolling_sharpe as "
            "create_rolling_sharpe"
        ),
        "from nautilus_trader.analysis.tearsheet import create_tearsheet as create_tearsheet",
        (
            "from nautilus_trader.analysis.tearsheet import create_tearsheet_from_stats as "
            "create_tearsheet_from_stats"
        ),
        (
            "from nautilus_trader.analysis.tearsheet import create_yearly_returns as "
        ...
        (
            "from nautilus_trader.analysis.tearsheet import register_tearsheet_chart as "
...
def python_libdir_env() -> dict[str, str]:
    """
    Return env vars so linked binaries can find this interpreter's libpython.
...
def resolve_python_stub_root(pyproject: dict) -> Path:
    ...
    try:
        ...
        raise RuntimeError("python-source must stay within the python/ package directory") from e
...
def generate_stubs() -> bool:
    ...
    targets = filtered or stub_files
...
def _extract_pyo3_signature_defaults(attr: str) -> dict[str, str] | None:
    """
    Parse a ``#[pyo3(signature = (...))]`` attribute into a parameter-defaults mapping.
...
def _split_signature_params(params_str: str) -> list[str]:
    """
    Split a pyo3 signature parameter list, respecting nested brackets.

    Nested parens and angle brackets keep defaults like ``HashMap::new()`` intact.
...
# Literal Rust default values with direct Python equivalents
LITERAL_DEFAULTS = {
    "None": "None",
    "true": "True",
    "false": "False",
...
def _rust_default_to_python(rust_default: str) -> str | None:
    ...
    """
    if rust_default in LITERAL_DEFAULTS:
        return LITERAL_DEFAULTS[rust_default]
...
def collect_rust_class_fixups(workspace_root: Path) -> dict[str, ClassMethodFixup]:
    """
    Collect getter, staticmethod, and signature defaults from Rust PyO3.
    """
...
def collect_renamed_enums(workspace_root: Path) -> set[str]:
    """
    Collect enums whose pyclass uses ``rename_all = "SCREAMING_SNAKE_CASE"``.
    """
    renamed: set[str] = set()
...
def collect_renamed_enum_variants(workspace_root: Path) -> dict[str, list[str]]:
    """
    Collect Rust variant names for enums with ``rename_all = "SCREAMING_SNAKE_CASE"``.
    """
...
def rename_enum_variants(
    ...
    """
    Rename stub enum variants from PascalCase to SCREAMING_SNAKE_CASE.

    Applies to enums whose pyclass has ``rename_all = "SCREAMING_SNAKE_CASE"``.
...
def fix_enum_defaults_in_signatures(content: str, renamed_enums: set[str]) -> str:
    """
    Fix enum variant names in defaults from ``apply_signature_defaults``.
...
def _register_free_function_defaults(
    ...
    python_name = _python_exposed_name(rust_name, attrs, is_getter=False)
    if not python_name:
        return
    for attr in attrs:
...
def register_rust_method_fixup(
    ...
    """
    Register getter, staticmethod, classmethod, and signature defaults.
    """
    rust_name = method_match.group(1)
    ...
    python_name = _python_exposed_name(rust_name, attrs, is_getter=is_getter)
    if not python_name:
        return

    fallback_name = _python_exposed_name(rust_name, [], is_getter=False)
...
def register_rust_setter_fixup(
    ...
    """
    Register setter property metadata for a Rust setter method.
    """
...
def render_missing_classmethod_stub(
    ...
    return (
        f"    @classmethod\n    def {method_name}(cls, {param_name}: typing.Any) -> "
        f"{class_name}: ..."
    )
...
def rename_stub_classes(content: str, class_fixups: dict[str, ClassMethodFixup]) -> str:
    """
    Rename generated classes to the public ``#[pyclass(name = ...)]`` names.
    """
    class_renames = {
...
def apply_class_block_fixups(
    ...
    """
    Apply fixups to every method block within a stub class block.
    """
...
def render_stub_property_setter(property_name: str, method_block: list[str]) -> list[str]:
    """
    Render a property setter block from a stub method block.
    """
    decorators, signature_text, remainder = split_method_block(method_block)
...
def drop_stub_param_default(signature_text: str, param_name: str) -> str:
    """
    Drop the default value from a named parameter in a stub signature.
    """
    pattern = re.compile(
...
def merge_stub_content(existing: str, new_blocks: list[str], fixup: StubFixup) -> str:
    ...
    content = "\n".join(parts)
    return normalize_stub_content(content)
...
def rename_methods(content: str) -> str:
    ...
    return _escape_keyword_variants(content)
...
def _is_optional_param(param: str) -> bool:
    ...
    if colon < 0:
        return False
    type_part = param[colon + 1 :].split("=", maxsplit=1)[0].strip()
...
def _build_defaults_lookup(
    ...
    Accounts for pyclass name renames.
...
def _scan_param_default_end(line: str, start: int) -> int:
    ...
    for i in range(start, len(line)):
        ...
        if quote is not None:
            quote, escaped = _advance_quoted_default(ch, quote, escaped=escaped)
            continue
...
def _qualify_enum_default(py_default: str, type_text: str) -> str:
    """
    Qualify an enum default with its module prefix when the type uses a qualified form.
...
def _validate_stub_exports(stub_content: str, exports: list[str], stub_path: Path) -> None:
    ...
    if missing:
        raise ValueError(f"{stub_path}: runtime __all__ names missing from stub: {missing}")
...
def main() -> int:
    """
    Parse CLI arguments and run the requested build action.
    """
